...
function AdminPanel({ employees, setEmployees, onExit }) {
  ...
  const enriched = employees
    .map((e) => {
      ...
      return { ...e, fs, fr, fBonus: fs.reduce((s, x) => s + x.bonus, 0) + fr.length * REVIEW_BONUS, fRevenue: fs.reduce((s, x) => s + x.price, 0), fSaleCount: fs.length, fReviewCount: fr.length, totalBonus: e.sales.reduce((s, x) => s + x.bonus, 0) + (e.reviews || []).length * REVIEW_BONUS, totalRevenue: e.sales.reduce((s, x) => s + x.price, 0) };
    })
    .sort((a, b) => b.fBonus - a.fBonus);

  const totals = { revenue: enriched.reduce((s, e) => s + e.fRevenue, 0), bonus: enriched.reduce((s, e) => s + e.fBonus, 0), sales: enriched.reduce((s, e) => s + e.fSaleCount, 0), reviews: enriched.reduce((s, e) => s + e.fReviewCount, 0) };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(145deg, #0d0d0d 0%, #0f0a14 30%, #0d0d0d 70%, #14100a 100%)", fontFamily: "'Nunito', sans-serif", color: "#e8e0d4", maxWidth: 520, margin: "0 auto", padding: "20px 20px 40px" }}>
      ...
      {showEditModal && (
        <div onClick={() => setShowEditModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 9000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "linear-gradient(145deg, #1a1410, #0d0d0d)", border: "1px solid rgba(212,175,55,0.2)", borderRadius: 24, padding: 28, width: "100%", maxWidth: 400 }}>
      ...
      {confirmDelete && (
        <div onClick={() => setConfirmDelete(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 9000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "linear-gradient(145deg, #1a1410, #0d0d0d)", border: "1px solid rgba(255,80,80,0.2)", borderRadius: 24, padding: 28, width: "100%", maxWidth: 400 }}>
      ...
      {adminView === "dashboard" && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
            {[{ l: "Выручка", v: formatMoney(totals.revenue), i: "💰", c: "#e8e0d4" }, { l: "Бонусы", v: formatMoney(totals.bonus), i: "🎁", c: "#d4af37" }, { l: "Продажи", v: totals.sales + " шт.", i: "🧾", c: "#6fa8dc" }, { l: "Отзывы", v: totals.reviews + " шт.", i: "⭐", c: "#e07a3a" }].map((x, i) => (
          ...
          {enriched.map((emp, i) => { const r = getRank(emp.totalBonus); const medals = ["🥇", "🥈", "🥉"]; return (
            <div key={emp.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", marginBottom: 8, ...cardStyle }}>
              ...
              <div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: "0.9rem" }}>{emp.name}</div><div style={{ fontSize: "0.7rem", color: r.color }}>{r.icon} {r.title} · {emp.fSaleCount} продаж · {emp.fReviewCount} отзывов</div></div>
          ...
          {enriched.map((emp) => { const r = getRank(emp.totalBonus); return (
            <div key={emp.id} style={{ ...cardStyle, borderRadius: 18, padding: 16, marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", gap: 6, fontSize: "0.7rem", color: "#6a6058", marginBottom: 12 }}>
                <span>📊 {emp.sales.length} продаж</span><span>·</span><span>⭐ {(emp.reviews || []).length} отзывов</span><span>·</span><span>💰 {formatMoney(emp.totalRevenue)}</span>
              </div>
      ...
      {adminView === "employees" && sel && (
        <div>
          ...
          <div style={{ background: "linear-gradient(145deg, rgba(212,175,55,0.1), rgba(20,16,12,0.95))", border: "1px solid rgba(212,175,55,0.15)", borderRadius: 24, padding: 24, marginBottom: 20, textAlign: "center" }}>
...
export default function HookahSalesApp() {
  ...
  const totalBonus = employee
    ? employee.sales.reduce((sum, s) => sum + s.bonus, 0) + (employee.reviews || []).length * REVIEW_BONUS
    : 0;
  ...
  const leaderboard = [...employees]
    .map((e) => ({
      ...e,
      totalBonus: e.sales.reduce((s, sale) => s + sale.bonus, 0) + (e.reviews || []).length * REVIEW_BONUS,
  ...
  return (
    <>
      ...
      <div
        style={{
          minHeight: "100vh",
          background: "linear-gradient(145deg, #0d0d0d 0%, #1a1410 30%, #0d0d0d 70%, #141018 100%)",
      ...
      >
        ...
        <div
          style={{
            ...
            background: "radial-gradient(ellipse at 30% 20%, rgba(212,175,55,0.03) 0%, transparent 50%), radial-gradient(ellipse at 70% 80%, rgba(139,90,43,0.04) 0%, transparent 50%)",
            pointerEvents: "none",
          }}
        />
        ...
        {!currentEmployee ? (
          <div style={{ padding: "40px 24px", animation: "slideUp 0.5s ease" }}>
            ...
            {showAdminLogin && (
              ...
              >
                <div onClick={(e) => e.stopPropagation()} style={{ background: "linear-gradient(145deg, #1a1410, #0d0d0d)", border: "1px solid rgba(212,175,55,0.2)", borderRadius: 24, padding: 28, width: "100%", maxWidth: 360, animation: "slideUp 0.3s ease" }}>
              ...
              <div style={{ marginBottom: 32 }}>
                ...
                {employees.map((emp) => {
                  ...
                  return (
                    <button
                      key={emp.id}
                      onClick={() => { setCurrentEmployee(emp.id); setView("main"); }}
                      style={{
                        display: "flex",
                        alignItems: "center",
                        gap: 14,
                        ...
                        padding: "16px 20px",
                        marginBottom: 10,
                        background: "linear-gradient(135deg, rgba(212,175,55,0.08), rgba(30,25,18,0.9))",
            ...
            >
              ...
              <button
                ...
                style={{
                  width: "100%",
                  padding: "14px",
                  background: newName.trim() ? "linear-gradient(135deg, #d4af37, #b8942d)" : "rgba(255,255,255,0.05)",
                  border: "none",
                  borderRadius: 12,
                  color: newName.trim() ? "#0d0d0d" : "#5a5248",
                  fontSize: "1rem",
                  fontWeight: 800,
          ...
          <>
                ...
                {pendingSaleItem && (
                  ...
                  >
                    <div
                      ...
                      style={{
                        background: "linear-gradient(145deg, #1a1410, #0d0d0d)",
                        border: "1px solid rgba(212,175,55,0.2)",
                        borderRadius: 24,
                        padding: 28,
                        width: "100%",
            ...
            {view === "stats" && (
              <div style={{ padding: "0 20px 40px", animation: "slideUp 0.3s ease" }}>
                <div
                  style={{
                    background: "linear-gradient(145deg, rgba(212,175,55,0.1), rgba(20,16,12,0.95))",
                    border: "1px solid rgba(212,175,55,0.15)",
                    borderRadius: 24,
                    padding: 24,
                    marginBottom: 20,
