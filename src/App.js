...
export default function HookahSalesApp() {
  ...
  // Save to cloud when employees change
  useEffect(() => {
    if (loading) return;
    saveToCloud({ employees });
    // Also save full data with photos locally
    saveLocalPhotos({ employees });
  }, [employees]);
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#0a0a0f", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Nunito', sans-serif" }}>
        <div style={{ fontSize: "3.5rem", marginBottom: 16, animation: "gentleFloat 2s ease-in-out infinite" }}>🌬️</div>
        <div style={{ color: "#d4af37", fontSize: "1rem", fontWeight: 700, letterSpacing: 2 }}>ЗАГРУЗКА...</div>
        <div style={{ color: "#5a5248", fontSize: "0.75rem", marginTop: 8 }}>Подключение к облаку</div>
