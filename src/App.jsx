...
// ── Badges (Duolingo/Starbucks-style collectible badges) ──
const BADGES = [
  { id: "b_first_day", title: "Новичок", desc: "Первый рабочий день", icon: "🌱", condition: (emp) => (emp.sales || []).length >= 1 },
  { id: "b_10_sales", title: "Стартер", desc: "10 продаж", icon: "🎯", condition: (emp) => (emp.sales || []).length >= 10 },
  { id: "b_50_sales", title: "Профи", desc: "50 продаж", icon: "⭐", condition: (emp) => (emp.sales || []).length >= 50 },
  { id: "b_100_sales", title: "Мастер", desc: "100 продаж", icon: "💫", condition: (emp) => (emp.sales || []).length >= 100 },
  { id: "b_250_sales", title: "Элита", desc: "250 продаж", icon: "🔱", condition: (emp) => (emp.sales || []).length >= 250 },
  { id: "b_500_sales", title: "Легенда", desc: "500 продаж", icon: "👑", condition: (emp) => (emp.sales || []).length >= 500 },
  { id: "b_5_reviews", title: "Коммуникатор", desc: "5 отзывов", icon: "💬", condition: (emp) => (emp.reviews || []).length >= 5 },
  { id: "b_20_reviews", title: "Дипломат", desc: "20 отзывов", icon: "🎖️", condition: (emp) => (emp.reviews || []).length >= 20 },
  { id: "b_revenue_50k", title: "Полтинник", desc: "50 000₽ выручки", icon: "💎", condition: (emp) => (emp.sales || []).reduce((s, x) => s + x.price, 0) >= 50000 },
  { id: "b_revenue_100k", title: "Сотка", desc: "100 000₽ выручки", icon: "💰", condition: (emp) => (emp.sales || []).reduce((s, x) => s + x.price, 0) >= 100000 },
  { id: "b_revenue_500k", title: "Магнат", desc: "500 000₽ выручки", icon: "🏦", condition: (emp) => (emp.sales || []).reduce((s, x) => s + x.price, 0) >= 500000 },
  { id: "b_streak_3", title: "Три дня подряд", desc: "Серия 3 дня", icon: "🔥", condition: (emp) => getStreak(emp) >= 3 },
  { id: "b_streak_7", title: "Неделя огня", desc: "Серия 7 дней", icon: "🔥", condition: (emp) => getStreak(emp) >= 7 },
  { id: "b_streak_14", title: "Марафонец", desc: "Серия 14 дней", icon: "🌟", condition: (emp) => getStreak(emp) >= 14 },
  { id: "b_streak_30", title: "Месяц без пропусков", desc: "Серия 30 дней", icon: "🏆", condition: (emp) => getStreak(emp) >= 30 },
  { id: "b_night_owl", title: "Ночная сова", desc: "Продажа после полуночи", icon: "🦉", condition: (emp) => (emp.sales || []).some(s => { const h = new Date(s.timestamp).getHours(); return h >= 0 && h < 5; }) },
  { id: "b_speed_demon", title: "Молния", desc: "5 продаж за 15 мин", icon: "⚡", condition: (emp) => {
    const sales = (emp.sales || []).map(s => new Date(s.timestamp).getTime()).sort((a,b) => a-b);
    for (let i = 0; i <= sales.length - 5; i++) { if (sales[i+4] - sales[i] < 900000) return true; }
    return false;
  }},
];
...
function getStreak(emp) {
  if (!emp || !emp.sales || emp.sales.length === 0) return 0;
  const dates = [...new Set((emp.sales || []).map(s => new Date(s.timestamp).toDateString()))].sort((a, b) => new Date(b) - new Date(a));
  let streak = 1;
...
function getMotivation(todaySalesCount, streak, hour) {
  if (hour < 12) {
    const msgs = ["Доброе утро! Готов к продажам? 🌅", "Новый день — новые рекорды! ☀️", "Утро начинается с продаж! ☕"];
    return msgs[todaySalesCount % msgs.length];
  }
  if (todaySalesCount === 0) return "Первая продажа ждёт! Начни прямо сейчас 🚀";
  if (todaySalesCount >= 15) return "НЕВЕРОЯТНО! Ты машина! 🤖🔥";
  if (todaySalesCount >= 10) return "Ты в ударе! Не останавливайся! 💪";
  if (todaySalesCount >= 5) return "Отличный темп! Продолжай жечь! 🔥";
  if (streak >= 7) return streak + " дней подряд! Огненная серия! 🔥";
  if (streak >= 3) return "Серия " + streak + " дней! Не сломай её! ⚡";
  return "Каждая продажа приближает к цели! 💫";
}

// ── Personal Best Calculator ──
function getPersonalBests(emp) {
  if (!emp || !emp.sales) return { bestDaySales: 0, bestDayRevenue: 0, bestDayBonus: 0 };
  const byDate = {};
  (emp.sales || []).forEach(s => {
    const d = new Date(s.timestamp).toDateString();
    if (!byDate[d]) byDate[d] = { count: 0, revenue: 0, bonus: 0 };
    byDate[d].count++;
    byDate[d].revenue += s.price;
    byDate[d].bonus += s.bonus;
  });
  const days = Object.values(byDate);
  return {
    bestDaySales: Math.max(0, ...days.map(d => d.count)),
    bestDayRevenue: Math.max(0, ...days.map(d => d.revenue)),
    bestDayBonus: Math.max(0, ...days.map(d => d.bonus)),
  };
}

// ── XP Level System ──
function getLevel(totalBonus) {
  const level = Math.floor(totalBonus / 5000) + 1;
  const xpInLevel = totalBonus % 5000;
  const xpNeeded = 5000;
  return { level, xpInLevel, xpNeeded, percent: (xpInLevel / xpNeeded) * 100 };
}

// ── iiko Cloud API (через Cloudflare Worker) ──
const IIKO_WORKER_URL = "https://iiko-api.mishasimf893.workers.dev/";
...
async function saveToCloud(data) {
  ...
  try {
    ...
    await setDoc(DATA_DOC, sanitized);
  } catch (err) {
...
// ═══ Login Screen ═══
function LoginScreen({ employees, onPinLogin }) {
  ...
  const handlePinLogin = () => {
    ...
    const hash = simpleHash(pinCode.trim());
    const emp = employees.find(e => e.pinHash === hash);
    if (emp) {
      onPinLogin(emp.id);
      setPinCode("");
    } else {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0b1a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif", padding: 20 }}>
      ...
      {/* Mode toggle */}
      <div style={{ display: "flex", gap: 4, marginBottom: 20, width: "100%", maxWidth: 340 }}>
        <button onClick={() => setMode("pin")} style={{ flex: 1, padding: "10px 8px", borderRadius: 10, cursor: "pointer", fontSize: "0.8rem", fontWeight: 700, fontFamily: "'DM Sans', sans-serif", background: mode === "pin" ? "rgba(196,113,245,0.2)" : "rgba(255,255,255,0.06)", border: mode === "pin" ? "1px solid rgba(196,113,245,0.3)" : "1px solid rgba(255,255,255,0.08)", color: mode === "pin" ? "#c471f5" : "#6b7094" }}>👤 Сотрудник</button>
        <button onClick={() => setMode("admin")} style={{ flex: 1, padding: "10px 8px", borderRadius: 10, cursor: "pointer", fontSize: "0.8rem", fontWeight: 700, fontFamily: "'DM Sans', sans-serif", background: mode === "admin" ? "rgba(249,115,22,0.2)" : "rgba(255,255,255,0.06)", border: mode === "admin" ? "1px solid rgba(249,115,22,0.3)" : "1px solid rgba(255,255,255,0.08)", color: mode === "admin" ? "#f97316" : "#6b7094" }}>👑 Руководитель</button>
      </div>
        ...
        {mode === "pin" ? (
          <>
            <input
              ...
              type="password" placeholder="Введите PIN-код" autoComplete="off" inputMode="numeric"
              style={{ width: "100%", padding: "14px 16px", marginBottom: 10, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(196,113,245,0.2)", borderRadius: 12, color: "#eef0ff", fontSize: "1.2rem", fontFamily: "'DM Sans', sans-serif", outline: "none", boxSizing: "border-box", textAlign: "center", letterSpacing: 8 }}
            />
            {pinError && (
              <div style={{ color: "#ff5050", fontSize: "0.8rem", textAlign: "center", marginBottom: 10, padding: "8px 12px", background: "rgba(255,80,80,0.08)", borderRadius: 10 }}>{pinError}</div>
            )}
            <button onClick={handlePinLogin} style={{ width: "100%", padding: 16, background: pinCode ? "linear-gradient(135deg, #c471f5, #a855f7)" : "rgba(255,255,255,0.06)", border: "none", borderRadius: 14, color: pinCode ? "#0d0b1a" : "#4a4e6e", fontWeight: 800, cursor: pinCode ? "pointer" : "default", fontFamily: "'DM Sans', sans-serif", fontSize: "1rem" }}>
              Войти
            </button>
            {employees.length === 0 && (
              <div style={{ color: "#6b7094", fontSize: "0.75rem", textAlign: "center", marginTop: 12, padding: "8px 12px", background: "rgba(255,255,255,0.04)", borderRadius: 10 }}>Сотрудников пока нет. Войдите как руководитель и добавьте.</div>
            )}
          </>
        ) : (
          <>
            <input
              ...
              style={{ width: "100%", padding: "14px 16px", marginBottom: 10, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(249,115,22,0.2)", borderRadius: 12, color: "#eef0ff", fontSize: "0.95rem", fontFamily: "'DM Sans', sans-serif", outline: "none", boxSizing: "border-box" }}
            />
            <input
...
function AdminPanel({ employees, setEmployees, onExit, salesPlans, setSalesPlans, dailyQuests, setDailyQuests, adminPinHash, setAdminPinHash, menuCategories, setMenuCategories, bonusPercent, setBonusPercent }) {
  ...
  const changePassword = (id) => { if (!newEmpPassword.trim()) return; setEmployees((p) => p.map((e) => (e.id === id ? { ...e, pinHash: simpleHash(newEmpPassword.trim()), passwordHash: simpleHash(newEmpPassword.trim()), password: undefined } : e))); setShowPasswordModal(null); setNewEmpPassword(""); };
    ...
    <div style={{ minHeight: "100vh", background: "#0d0b1a", fontFamily: "'DM Sans', sans-serif", color: "#eef0ff", maxWidth: 520, margin: "0 auto", padding: "20px 20px 40px" }}>
      ...
      {adminView === "employees" && !selectedEmpId && (
        <div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            <input value={newEmpNameInput} onChange={e => setNewEmpNameInput(e.target.value)} placeholder="Имя сотрудника" style={{ flex: 1, padding: "12px 14px", borderRadius: 12, border: "1px solid rgba(196,113,245,0.2)", background: "rgba(255,255,255,0.08)", color: "#eef0ff", fontSize: "0.85rem", fontFamily: "'DM Sans', sans-serif", outline: "none" }} />
            <button onClick={() => {
              const name = newEmpNameInput.trim();
              if (!name) return;
              const id = "emp_" + Date.now() + "_" + Math.random().toString(36).slice(2, 6);
              setEmployees(prev => [...prev, { id, name, sales: [], reviews: [], shifts: [], pinHash: null, bonusMultiplier: 0, authEmail: "" }]);
              setNewEmpNameInput("");
            }} style={{ padding: "12px 20px", borderRadius: 12, cursor: "pointer", fontSize: "0.85rem", fontWeight: 800, fontFamily: "'DM Sans', sans-serif", background: "linear-gradient(135deg, #c471f5, #a855f7)", border: "none", color: "#0d0b1a", whiteSpace: "nowrap" }}>+ Добавить</button>
          </div>
      ...
      {adminView === "menu" && (
        <div>
          ...
          {/* iiko Sync Button */}
          <div style={{ background: "rgba(34,211,238,0.08)", border: "1px solid rgba(34,211,238,0.2)", borderRadius: 16, padding: 16, marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
              <div>
                <div style={{ fontWeight: 700, color: "#eef0ff", fontSize: "0.9rem" }}>🔄 Синхронизация с iiko</div>
                <div style={{ fontSize: "0.7rem", color: "#6b7094", marginTop: 4 }}>Загрузить меню из iiko Cloud (Nargilia Lounge)</div>
              </div>
              <button onClick={async () => {
                const btn = window.event?.target;
                if (btn) btn.textContent = "⏳ Загрузка...";
                try {
                  const res = await fetch(IIKO_WORKER_URL);
                  const data = await res.json();
                  if (data.success && data.menu) {
                    const iikoMenu = {};
                    Object.entries(data.menu).forEach(([key, cat]) => {
                      iikoMenu[key] = {
                        name: cat.name,
                        emoji: cat.emoji || "📦",
                        items: (cat.items || []).map(item => ({
                          id: item.id || (key.slice(0,2) + "_" + Math.random().toString(36).slice(2,8)),
                          name: item.name,
                          price: item.price || 0,
                        })),
                      };
                    });
                    setMenuCategories(iikoMenu);
                    alert("✅ Меню обновлено из iiko!\n\nКатегорий: " + data.menuCategoriesCount + "\nПозиций: " + data.menuItemsCount);
                  } else {
                    alert("❌ Ошибка: " + (data.error || "Пустой ответ"));
                  }
                } catch (err) {
                  alert("❌ Ошибка связи с iiko: " + err.message);
                }
                if (btn) btn.textContent = "🔄 Загрузить из iiko";
              }} style={{ padding: "10px 20px", borderRadius: 12, cursor: "pointer", fontSize: "0.8rem", fontWeight: 800, fontFamily: "'DM Sans', sans-serif", background: "linear-gradient(135deg, #22d3ee, #06b6d4)", border: "none", color: "#0d0b1a", boxShadow: "0 4px 15px rgba(34,211,238,0.3)" }}>🔄 Загрузить из iiko</button>
            </div>
          </div>
...
function EmployeeDashboard({
  ...
  // ═══ GAMIFICATION: Streak, Badges, Level, Personal Best ═══
  const streak = getStreak(employee);
  const earnedBadges = BADGES.filter(b => employee && b.condition(employee));
  const levelInfo = getLevel(totalBonus);
  const personalBests = getPersonalBests(employee);
  const motivation = getMotivation(todaySales.length, streak, new Date().getHours());
  const isNewBestSales = todaySales.length > 0 && todaySales.length >= personalBests.bestDaySales;
  const isNewBestRevenue = todayRevenue > 0 && todayRevenue >= personalBests.bestDayRevenue;
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes levelGlow {
          0%, 100% { box-shadow: 0 0 10px rgba(196,113,245,0.3); }
          50% { box-shadow: 0 0 25px rgba(196,113,245,0.6), 0 0 50px rgba(56,189,248,0.2); }
        }
      `}</style>
      ...
      {showBadgeModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 9600, display: "flex", flexDirection: "column", animation: "fadeIn 0.3s ease" }}>
          <div style={{ maxWidth: 480, width: "100%", margin: "0 auto", padding: "20px", overflowY: "auto", flex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              <div>
                <div style={{ fontFamily: "'Outfit', serif", fontSize: "1.5rem", fontWeight: 900, color: "#f0abfc" }}>
                  Коллекция значков
                </div>
                <div style={{ fontSize: "0.8rem", color: "#6b7094", marginTop: 4 }}>
                  {earnedBadges.length} из {BADGES.length} получено
                </div>
              </div>
              <button onClick={() => setShowBadgeModal(false)} style={{ background: "rgba(196,113,245,0.2)", border: "1px solid rgba(196,113,245,0.3)", borderRadius: 12, padding: "8px 16px", color: "#8b8fa3", cursor: "pointer", fontSize: "0.85rem", fontWeight: 600 }}>X</button>
            </div>
            <div style={{ height: 6, background: "rgba(255,255,255,0.06)", borderRadius: 3, marginBottom: 24, overflow: "hidden" }}>
              <div style={{ width: (earnedBadges.length / BADGES.length * 100) + "%", height: "100%", background: "linear-gradient(90deg, #c471f5, #38bdf8, #22d3ee)", borderRadius: 3, transition: "width 1s ease" }} />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {BADGES.map(badge => {
                const earned = earnedBadges.some(b => b.id === badge.id);
                return (
                  <div key={badge.id} style={{ background: earned ? "linear-gradient(135deg, rgba(196,113,245,0.15), rgba(56,189,248,0.08))" : "rgba(255,255,255,0.03)", border: earned ? "1px solid rgba(196,113,245,0.3)" : "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: "16px 14px", textAlign: "center", opacity: earned ? 1 : 0.4 }}>
                    <div style={{ fontSize: "2rem", marginBottom: 6, filter: earned ? "none" : "grayscale(100%)" }}>{badge.icon}</div>
                    <div style={{ fontSize: "0.8rem", fontWeight: 800, color: earned ? "#eef0ff" : "#4a4e6e", marginBottom: 2 }}>{badge.title}</div>
                    <div style={{ fontSize: "0.65rem", color: earned ? "#8b8fa3" : "#2e3254" }}>{badge.desc}</div>
                    {earned && (<div style={{ fontSize: "0.55rem", color: "#22d3ee", marginTop: 4, fontWeight: 700 }}>ПОЛУЧЕНО</div>)}
                  </div>
                );
              })}
            </div>
      ...
      {/* NEW BADGE POPUP */}
      {newBadgePopup && (
        <div style={{ position: "fixed", top: 120, left: "50%", transform: "translateX(-50%)", zIndex: 9850, animation: "slideUp 0.4s ease", background: "linear-gradient(135deg, rgba(20,18,40,0.97), rgba(30,25,55,0.97))", backdropFilter: "blur(24px)", border: "1px solid rgba(34,211,238,0.4)", borderRadius: 20, padding: "16px 24px", display: "flex", alignItems: "center", gap: 14, boxShadow: "0 8px 40px rgba(34,211,238,0.25)", maxWidth: 340, width: "90%" }}>
          <div style={{ fontSize: "2.2rem", animation: "badgeUnlock 0.6s ease" }}>{newBadgePopup.icon}</div>
          <div>
            <div style={{ fontSize: "0.6rem", color: "#22d3ee", textTransform: "uppercase", letterSpacing: 3, fontWeight: 700, marginBottom: 2 }}>Новый значок!</div>
            <div style={{ fontWeight: 800, fontSize: "1rem", color: "#eef0ff" }}>{newBadgePopup.title}</div>
            <div style={{ fontSize: "0.75rem", color: "#8b8fa3" }}>{newBadgePopup.desc}</div>
          </div>
        </div>
      )}
        ...
        {/* EMPLOYEE VIEW — always shown since auth check above */}
            ...
            {view === "main" && (
              <div style={{ padding: "0 20px 100px", animation: "slideUp 0.3s ease" }}>
                ...
                {/* STREAK & LEVEL BAR */}
                <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
                  <div style={{ flex: 1, background: streak >= 3 ? "linear-gradient(135deg, rgba(249,115,22,0.2), rgba(239,68,68,0.1))" : "linear-gradient(135deg, rgba(20,18,40,0.7), rgba(30,25,55,0.5))", border: streak >= 3 ? "1px solid rgba(249,115,22,0.3)" : "1px solid rgba(196,113,245,0.15)", borderRadius: 18, padding: "14px 16px", textAlign: "center" }}>
                    <div style={{ fontSize: streak >= 7 ? "1.8rem" : "1.4rem", animation: streak >= 3 ? "streakFlame 1.5s ease-in-out infinite" : "none", marginBottom: 4 }}>
                      {streak >= 3 ? "🔥" : streak >= 1 ? "⚡" : "💤"}
                    </div>
                    <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: "1.6rem", fontWeight: 900, color: streak >= 3 ? "#f97316" : streak >= 1 ? "#eef0ff" : "#4a4e6e" }}>{streak}</div>
                    <div style={{ fontSize: "0.6rem", color: streak >= 3 ? "#fb923c" : "#6b7094", textTransform: "uppercase", letterSpacing: 2, fontWeight: 700 }}>
                      {streak === 1 ? "день" : streak >= 2 && streak <= 4 ? "дня" : "дней"} подряд
                    </div>
                  </div>
                  <div style={{ flex: 2, background: "linear-gradient(135deg, rgba(20,18,40,0.7), rgba(30,25,55,0.5))", border: "1px solid rgba(196,113,245,0.15)", borderRadius: 18, padding: "14px 16px" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg, rgba(196,113,245,0.3), rgba(56,189,248,0.2))", border: "1px solid rgba(196,113,245,0.4)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Outfit', sans-serif", fontSize: "0.9rem", fontWeight: 900, color: "#f0abfc", animation: "levelGlow 3s ease-in-out infinite" }}>{levelInfo.level}</div>
                        <div>
                          <div style={{ fontSize: "0.65rem", color: "#6b7094", textTransform: "uppercase", letterSpacing: 2, fontWeight: 600 }}>Уровень</div>
                          <div style={{ fontSize: "0.85rem", fontWeight: 800, color: "#eef0ff" }}>{formatMoney(levelInfo.xpInLevel)} / {formatMoney(levelInfo.xpNeeded)} XP</div>
                        </div>
                      </div>
                      <div style={{ fontSize: "0.7rem", color: "#c471f5", fontWeight: 700 }}>{Math.round(levelInfo.percent)}%</div>
                    </div>
                    <div style={{ height: 6, background: "rgba(255,255,255,0.06)", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ width: levelInfo.percent + "%", height: "100%", background: "linear-gradient(90deg, #c471f5, #38bdf8)", borderRadius: 3, transition: "width 1s cubic-bezier(0.4, 0, 0.2, 1)", boxShadow: "0 0 8px rgba(196,113,245,0.4)" }} />
                    </div>
                  </div>
                </div>

                {/* MOTIVATION */}
                <div style={{ background: "linear-gradient(135deg, rgba(196,113,245,0.08), rgba(56,189,248,0.05))", border: "1px solid rgba(196,113,245,0.12)", borderRadius: 14, padding: "10px 16px", marginBottom: 16, textAlign: "center", fontSize: "0.8rem", fontWeight: 600, color: "#8b8fa3", fontStyle: "italic" }}>{motivation}</div>

                {/* PERSONAL BEST */}
                {isNewBestSales && todaySales.length > 1 && (
                  <div style={{ background: "linear-gradient(135deg, rgba(34,211,238,0.15), rgba(56,189,248,0.08))", border: "1px solid rgba(34,211,238,0.3)", borderRadius: 14, padding: "10px 16px", marginBottom: 16, textAlign: "center", animation: "newRecord 1s ease-in-out infinite" }}>
                    <span style={{ fontSize: "0.85rem", fontWeight: 800, color: "#22d3ee" }}>🏅 НОВЫЙ РЕКОРД! {todaySales.length} продаж за день!</span>
                  </div>
                )}

                {/* BADGES PREVIEW */}
                {earnedBadges.length > 0 && (
                  <div onClick={() => setShowBadgeModal(true)} style={{ background: "linear-gradient(135deg, rgba(20,18,40,0.7), rgba(30,25,55,0.5))", border: "1px solid rgba(196,113,245,0.15)", borderRadius: 18, padding: "12px 16px", marginBottom: 16, cursor: "pointer" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <span style={{ fontSize: "0.9rem" }}>🏅</span>
                        <span style={{ fontSize: "0.7rem", color: "#c471f5", textTransform: "uppercase", letterSpacing: 2, fontWeight: 700 }}>Значки</span>
                      </div>
                      <span style={{ fontSize: "0.7rem", color: "#6b7094", fontWeight: 600 }}>{earnedBadges.length}/{BADGES.length} →</span>
                    </div>
                    <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4 }}>
                      {earnedBadges.slice(0, 8).map(badge => (
                        <div key={badge.id} style={{ flexShrink: 0, width: 40, height: 40, borderRadius: 12, background: "rgba(196,113,245,0.15)", border: "1px solid rgba(196,113,245,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem" }}>{badge.icon}</div>
                      ))}
                      {earnedBadges.length > 8 && (
                        <div style={{ flexShrink: 0, width: 40, height: 40, borderRadius: 12, background: "rgba(196,113,245,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.7rem", color: "#6b7094", fontWeight: 700 }}>+{earnedBadges.length - 8}</div>
                      )}
                    </div>
                  </div>
                )}
...
export default function HookahSalesApp() {
  ...
  // 1. Loading (wait for data, auth can be optional for PIN login)
  if (loading) {
    return <LoadingScreen />;
  }

  // 2. Not logged in (and no PIN employee selected)
  if (!authUser && !currentEmployee) {
    return <LoginScreen employees={employees} onPinLogin={(empId) => setCurrentEmployee(empId)} />;
  }

  // 3. Admin (via Firebase Auth)
  if (isAdmin) {
    return (
      <AdminPanel
        employees={employees}
  ...
  // 4. Employee not linked (only for email-based auth)
  if (authUser && !isAdmin && !employee) {
    return <UnlinkedAccountScreen email={authUser.email} onLogout={handleLogout} />;
  }

  // 5. Employee dashboard (PIN or email auth)
  if (employee) {
    return (
    <EmployeeDashboard
      employee={employee}
      ...
      salesPlans={salesPlans}
      setSalesPlans={setSalesPlans}
      onLogout={() => { setCurrentEmployee(null); if (authUser) signOut(auth); }}
    />
  );
  }
